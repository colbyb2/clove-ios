...
| Gentle Plans remain separate from logged activities and analytics. | [Plan model](../Clove/Models/PacingPlanItem.swift), [plan repository](../Clove/Repos/PacingPlanRepo.swift), [Today plan interface](../Clove/Views/Today/PacingPlanTracker.swift) | Completing a plan changes only its plan state; it does not create an activity record. |
